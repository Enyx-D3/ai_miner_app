...
class Brain2MobilePairing {
  ...
  Brain2PairingInvite parse(String raw) {
    ...
    if (uri.scheme != 'http' && uri.scheme != 'https') {
      throw const FormatException('Brain2 pairing QR must contain an http/https URL.');
    }
    ...
    if (explicitSignal != null && explicitSignal.isNotEmpty) {
      ...
      if (signalUri.scheme != 'http' && signalUri.scheme != 'https') {
        throw const FormatException('Brain2 signaling origin in QR is invalid.');
      }
      signalingOrigin = Uri(
  ...
  Future<Brain2P2PSync> join(Brain2PairingInvite invite) async {
    ...
    if (token.isEmpty || root.isEmpty) {
      api.close();
      throw StateError('Brain2 signaling server returned an incomplete pairing response.');
    }
    ...
    if (protocol != 2) {
      api.close();
      throw StateError('Brain2 sync protocol mismatch: server=$protocol, mobile=2.');
    }
    ...
    if (localMessages == 0) {
      ...
      api.close();
      throw StateError(
        'This phone already contains a different Brain2 memory. '
        'Export/reset it before joining this QR.',
      );
    }
  ...
  Future<Brain2P2PSync?> restore() async {
    ...
    if (storedDeviceId.isNotEmpty && storedDeviceId != deviceId) {
      throw StateError('Stored Brain2 P2P credential belongs to another device identity.');
    }
    if (storedRoot.isNotEmpty && await db.memoryRoot() != storedRoot) {
      throw StateError('Stored Brain2 P2P credential belongs to another memory root.');
    }
