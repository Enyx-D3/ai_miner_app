...
List<String> _keywords(String value, int limit) {
  ...
  for (final token
      in normalizeText(value).toLowerCase().split(RegExp(r'[^a-z0-9.+#-]+'))) {
...
bool _titleIsGeneric(String title) {
  final value =
      normalizeText(title).replaceAll(RegExp(r'\s*\(\d{4}-\d{2}-\d{2}\)$'), '');
...
ProjectFingerprint fingerprintConversation(
...
List<String> _projectTerms(Map<String, Object?> project) {
  final aliases = ((project['aliases'] as List?) ?? const [])
      .map((e) => '$e')
      .expand((e) => _keywords(e, 8));
  final entities =
      ((project['entityTerms'] as List?) ?? const []).map((e) => '$e');
  ...
  return <String>{
    ...
    ...entityKeywords,
    ...aliases,
    ..._keywords('${project['summary'] ?? ''}', 12),
    ..._keywords('${project['name'] ?? ''}', 8)
  }.toList();
}

double scoreProject(
    ProjectFingerprint fingerprint, Map<String, Object?> project) {
  final terms = _projectTerms(project);
  final aliases =
      ((project['aliases'] as List?) ?? const []).map((e) => '$e').toList();
  final entities =
      ((project['entityTerms'] as List?) ?? const []).map((e) => '$e').toList();
  final normalized =
      fingerprint.allTerms.map((e) => normalizeText(e).toLowerCase()).toSet();
  final exact = [...aliases, ...entities]
          .any((e) => normalized.contains(normalizeText(e).toLowerCase()))
      ? 1.0
      : 0.0;
  final titleScore = fingerprint.genericTitle
      ? 0
      : _tokenOverlap(fingerprint.titleTerms, terms);
  ...
  for (final alias in aliases) {
    final score =
        _tokenOverlap(fingerprint.contentTerms.take(12), _keywords(alias, 8));
    if (score > aliasScore) aliasScore = score;
  }
  final evidence = [
    contentScore,
    _tokenOverlap(fingerprint.contentTerms.take(12), terms)
  ].reduce((a, b) => a > b ? a : b);
  final score = (fingerprint.genericTitle ? 0 : titleScore * .2) +
      evidence * .46 +
      entityScore * .18 +
      aliasScore * .08 +
      exact * .12;
...
ProjectResolution resolveProject({
  ...
  final ranked = existingProjects
      .map((project) =>
          (project: project, score: scoreProject(fingerprint, project)))
      .toList()
    ..sort((a, b) => b.score.compareTo(a.score) != 0
        ? b.score.compareTo(a.score)
        : '${b.project['updatedAt'] ?? ''}'
            .compareTo('${a.project['updatedAt'] ?? ''}'));
  if (ranked.isNotEmpty) {
    final best = ranked.first;
    final runner = ranked.length > 1 ? ranked[1] : null;
    final overlap = fingerprint.contentTerms
        .where(_projectTerms(best.project).toSet().contains)
        .length;
    final accepted = fingerprint.genericTitle
        ? best.score >= .14 &&
            overlap >= 3 &&
            (runner == null || best.score >= runner.score + .08)
        : best.score >= .34;
    if (accepted) {
      return ProjectResolution(
        ...
        aliases: <String>{
          ...((best.project['aliases'] as List?) ?? const []).map((e) => '$e'),
          normalizeText(title)
        }.where((e) => e.isNotEmpty).toList(),
        entityTerms: <String>{
          ...((best.project['entityTerms'] as List?) ?? const [])
              .map((e) => '$e'),
          ...fingerprint.entityTerms
        }.toList(),
  ...
  final key = <String>[
    ...fingerprint.titleTerms,
    ...fingerprint.contentTerms.take(12),
    ...fingerprint.entityTerms.take(8)
  ]..sort();
  final id = canonicalId('proj',
      ['brain2-project', key.isEmpty ? name.toLowerCase() : key.join('|')]);
  return ProjectResolution(
    projectId: id,
    name: name,
    slug: _slug(name),
    aliases: <String>{normalizeText(title), name}
        .where((e) => e.isNotEmpty)
        .toList(),
