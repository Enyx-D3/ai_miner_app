...
class MobileIntelligencePipeline {
  ...
  late final MobileEvidenceBlockEngine evidenceBlocks =
      MobileEvidenceBlockEngine(db, mutations);
  ...
  List<Map<String, Object?>> _atomize(
    ...
    for (var i = 0; i < chunks.length; i++) {
      ...
      final strictDecision = evaluateStrictCurrentTruthCandidate(
        ...
        timestampValid:
            '${message['occurredAt'] ?? message['createdAt'] ?? ''}'.isNotEmpty,
      );
  ...
  String _kind(String value) {
    ...
    if (RegExp(r'\b(hypothesis|maybe|perhaps|could|might)\b').hasMatch(text)) {
      return 'hypothesis';
    }
    return 'fact';
  }
  ...
  Future<void> refreshProjectSnapshots(String projectId) async {
    ...
    await evidenceBlocks.refreshProject(projectId);
    ...
    final intelligenceSnapshot =
        await intelligenceLayer.buildProjectIntelligence(
      projectId,
      truthRows: truthRows,
      atoms: atoms,
    );
    ...
    final notebookSnapshot = <String, Object?>{
      ...
      'nextActions':
          atoms.where((atom) => '${atom['kind']}' == 'task').take(12).toList(),
      'createdAt': now,
      'updatedAt': now,
      'schemaVersion': brain2SchemaVersion,
    };
