...
class Brain2ImportService {
  ...
  Future<ImportSummary> importPreparedJsonFile(
    ...
    final knownProjects =
        await db.records('projects', orderBy: 'updated_at DESC');
    ...
    await for (final conversationRaw in streamJsonObjectArrayFile(jsonPath)) {
      ...
      final existingProject =
          await db.getRecord('projects', resolution.projectId);
      ...
      final knownProjectIndex = knownProjects
          .indexWhere((item) => '${item['id']}' == '${project['id']}');
  ...
  String _provider(Map<String, Object?> conversation) {
    final raw =
        '${conversation['_brain2_provider'] ?? conversation['provider'] ?? 'chatgpt'}'
            .trim()
            .toLowerCase();
